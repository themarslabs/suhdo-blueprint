...
export type FileAsset = {
  ...
  caption?: string
  link?: string
  variants?: Partial<Record<"large" | "medium" | "thumb" | "icon", string>>
  size?: number
  width?: number | null
  height?: number | null
  mime?: string
...
export function FileTile({
  ...
  const metadata = [asset.width && asset.height ? `${asset.width}×${asset.height}` : null, typeof asset.size === "number" ? formatFileSize(asset.size) : null].filter(Boolean).join(" · ")

  return (
    <article className={cn("group relative flex h-full cursor-pointer flex-col overflow-hidden rounded-lg border bg-background transition-colors", isSelected ? "border-primary ring-1 ring-primary" : "border-border hover:border-foreground/30", className)}>
